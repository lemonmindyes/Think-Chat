...
if __name__ == '__main__':
    ...
    total_step = 100000 * accumulation_steps # iter 100K
    ...
    for step in range(checkpoint['current_step'], total_step):
        ...
        if step % 100 == 0:
            ...
            # Through its annual Federal Funding Opportunity, the NOAA Office of Ocean Exploration and Research funds projects designed to enhance ocean exploration and scientific knowledge of unknown or poorly known areas of the ocean through interdisciplinary exploration and to advance and disseminate knowledge of the ocean environment.\nThe table below lists projects that have been funded via this competitive opportunity from Fiscal Year (FY) 2014 through present, sortable by year funded, project title, principal investigator (PI) name, affiliation, and primary project topic/focus.\nTo sort multiple columns simultaneously, select the desired columns while holding down the SHIFT key.
            text = f'<im_start>Through its annual Federal Funding Opportunity, the NOAA Office of Ocean Exploration and Research funds projects designed'
        ...
        if step % 1000 == 0:
            ...
            torch.save(checkpoint, 'pretrain_checkpoint.bin.pt')
            os.replace('pretrain_checkpoint.bin.pt', 'pretrain_checkpoint.bin')
